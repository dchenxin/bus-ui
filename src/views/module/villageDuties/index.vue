<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['module:villageDuties:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['module:villageDuties:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['module:villageDuties:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['module:villageDuties:export']"
        >导出</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table v-loading="loading" :data="villageDutiesList" @selection-change="handleSelectionChange">
...
<!--          <el-input v-model="form.age" placeholder="请输入年龄" />-->
        ...
        <el-form-item label="任职年数" prop="dutyYear" v-if="form.duty !=='村民'">
...
<script>
import { listVillageDuties, getVillageDuties,
  delVillageDuties, addVillageDuties,
  updateVillageDuties, exportVillageDuties,
  importTemplate } from "@/api/module/villageDuties";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/module/villageDuties/importData",
      },
  ...
  methods: {
   ...
   /* getRuralList(){
    ...
    getList() {
      this.loading = true;
      listVillageDuties(this.queryParams).then(response => {
        this.villageDutiesList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getVillageDuties(id).then(response => {
          ...
          this.$refs["form"].validate(valid => {
              if (valid) {
                  if (this.form.id != undefined) {
                      updateVillageDuties(this.form).then(response => {
                      ...
                      addVillageDuties(this.form).then(response => {
                          if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除乡村职责信息为"' + row.name + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function() {
          return delVillageDuties(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有乡村职责信息数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportVillageDuties(queryParams);
        }).then(response => {
          this.download(response.msg);
        }).catch(function() {});
    }
