<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          v-if="checkRole(['commonUser']) || checkRole(['admin'])"
          type="primary"
          icon="el-icon-plus"
          size="mini"
          @click="handleAdd"
          v-hasPermi="['module:pubilcFeedback:add']"
        >发布</el-button>
      </el-col>
      <!--<el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['module:pubilcFeedback:edit']"
        >修改</el-button>
      </el-col>-->
      <!--<el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['module:pubilcFeedback:remove']"
        >删除</el-button>
      </el-col>-->
      <!--<el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['module:pubilcFeedback:export']"
        >导出</el-button>
      </el-col>-->
    </el-row>

    <el-table v-loading="loading" :data="pubilcFeedbackList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="回复人" align="center" prop="content" >
        <template slot-scope="scope">
          <span v-if="scope.row.returnerId!=null && scope.row.returnerId!==''">{{scope.row.returnerName}}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>
      <el-table-column label="回复内容" align="center" prop="content" width="200px" show-overflow-tooltip>
        <template slot-scope="scope">
          <span v-if="scope.row.returnContent!=null && scope.row.returnContent!==''">{{scope.row.returnContent}}</span>
          <span v-else>-</span>
        </template>
      </el-table-column>

      <el-table-column label="浏览量" align="center" prop="viewNum" />
      <el-table-column label="状态" align="center" prop="state" v-if="!checkRole(['commonUser'])" >
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            v-if="(scope.row.returnContent==null || scope.row.returnContent==='') && !checkRole(['commonUser'])"
            size="mini"
            type="text"
          ...
          >回复</el-button>
          <el-button
            v-if="scope.row.state==='examine' && !checkRole(['commonUser'])"
            size="mini"
            type="text"
            icon="el-icon-s-check"
    ...
    <el-dialog :title="title" :visible.sync="detailOpen" width="900px" append-to-body>
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <el-form-item label="回复人" prop="content">
          <div style="padding: 0 10px;">
            <span v-if="form.returnerId!=null && form.returnerId!==''" >{{form.returnerName}}</span>
            <span v-else >-</span>
          </div>
        </el-form-item>
        ...
        <el-form-item label="回复内容" prop="content">
          <div style="padding: 0 10px;">
            <span v-if="form.returnContent!=null && form.returnContent!==''" >{{form.returnContent}}</span>
            <span v-else >-</span>
          </div>
        </el-form-item>
    ...
    <!-- 回复民情反馈 对话框 -->
    <el-dialog :title="title" :visible.sync="returnOpen" width="500px" :close-on-click-modal="false" append-to-body>
      <el-input type="textarea"
        :rows="6" placeholder="请输入内容"
        v-model="form.returnContent">
      </el-input>
      <div slot="footer" class="dialog-footer">
...
<script>
import { listPubilcFeedback, getPubilcFeedback,
  delPubilcFeedback, addPubilcFeedback, updatePubilcFeedback,
  exportPubilcFeedback, putFeedback, offFeedback, setReply
} from "@/api/module/pubilcFeedback";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listPubilcFeedback(this.queryParams).then(response => {
        this.pubilcFeedbackList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getPubilcFeedback(id).then(response => {
    ...
    // 提交回复内容
    submitReply(){
      setReply(this.form.id,this.form.returnContent).then(response => {
    ...
    handleExamine(row) {
      const id = row.id;
      this.$confirm('是否确认审核通过民情反馈：' + row.title , "审核", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "info"
      }).then(function() {
        return putFeedback(id);
      }).then(() => {
        this.getList();
    ...
    handlePut(row) {
      const id = row.id;
      this.$confirm('是否确认上架民情反馈：' + row.title , "上架", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "info"
      }).then(function() {
    ...
    handleOff(row) {
      const id = row.id;
      this.$confirm('是否确认下架民情反馈：' + row.title , "下架", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "info"
      }).then(function() {
        return offFeedback(id);
      }).then(() => {
        this.getList();
              ...
              if (valid) {
                ...
                this.form.image =JSON.stringify(images);
                  ...
                  if (this.form.id != undefined) {
                      updatePubilcFeedback(this.form).then(response => {
                          if (response.code === 200) {
                    ...
                    this.form.image =JSON.stringify(images);
                      ...
                      addPubilcFeedback(this.form).then(response => {
                          if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除民情反馈编号为"' + ids + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delPubilcFeedback(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有民情反馈数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportPubilcFeedback(queryParams);
        }).then(response => {
