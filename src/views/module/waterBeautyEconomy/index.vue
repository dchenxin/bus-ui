    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['module:waterBeautyEconomy:add']"
        >发布</el-button>
      </el-col>
      <!--<el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['module:waterBeautyEconomy:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['module:waterBeautyEconomy:remove']"
        >删除</el-button>
      </el-col>-->
      <!--<el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['module:waterBeautyEconomy:export']"
        >导出</el-button>
      </el-col>-->
    </el-row>

    <el-table v-loading="loading" :data="waterBeautyEconomyList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="900px" :close-on-click-modal="false" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <!--<el-form-item label="内容" prop="content">
          &lt;!&ndash;<el-input v-model="form.content" placeholder="请输入图文内容" />&ndash;&gt;
        ...
        <!--<el-form-item label="内容2" prop="content">
          <editor height="200px" v-model="form.content"></editor>
        </el-form-item>-->
        <el-form-item label="简介" prop="introduce">
          <el-input type="textarea" :rows="2" v-model="form.introduce" maxlength="30" placeholder="请输入简介，30字以内" />
        </el-form-item>
        <el-form-item label="内容" prop="content">
          <rich-editor :height="richHeight" v-model="form.content"/>
        </el-form-item>
      </el-form>
...
<script>
import {
  listWaterBeautyEconomy, getWaterBeautyEconomy,
  delWaterBeautyEconomy, addWaterBeautyEconomy, updateWaterBeautyEconomy,
  exportWaterBeautyEconomy, changeNewStatus
} from "@/api/module/waterBeautyEconomy";
import { checkPermi, checkRole } from "@/utils/permission";
import Editor from "../../../components/Editor/index";
import RichEditor from "../../components/richEditor/index"; // 权限判断函数

export default {
  name: "WaterBeautyEconomy",
  components: {RichEditor, Editor},
  data() {
    return {
      richHeight: 300, // 富文本框高度
      // 遮罩层
      loading: true,
      // 选中数组
      ids: [],
  ...
  methods: {
    ...
    handleDetail(row) {
      this.reset();
      const id = row.id;
      getWaterBeautyEconomy(id).then(response => {
              ...
              if (valid) {
                  if (this.form.id != undefined) {
                    ...
                    console.log(this.form.video);
                      ...
                      updateWaterBeautyEconomy(this.form).then(response => {
                    ...
                    console.log(this.form.video);
                      ...
                      addWaterBeautyEconomy(this.form).then(response => {
                          if (response.code === 200) {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除水美资讯 "' + row.title + '" 的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delWaterBeautyEconomy(ids);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有水美经济数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return exportWaterBeautyEconomy(queryParams);
        }).then(response => {
