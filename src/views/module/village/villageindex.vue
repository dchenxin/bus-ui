    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['module:village:villageAdd']"
        >发布</el-button>
      </el-col>
      <!--<el-col :span="1.5">
        <el-button
...
<script>
  ...
  import {changeNewStatus,offVillages,putVillages,villagelist, getVillage, delVillage, villageAdd, updateVillage, talentExport} from "@/api/module/village";
...
//============================================================
      ...
      getList() {
        this.loading = true;
        villagelist(this.queryParams).then(response => {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != undefined) {
              ...
              this.form.img = JSON.stringify(images);
              this.form.video = JSON.stringify(videos);
              villageAdd(this.form).then(response => {
                if (response.code === 200) {
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有人才振兴信息数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
          return talentExport(queryParams);
        }).then(response => {
