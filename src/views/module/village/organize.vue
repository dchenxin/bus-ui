    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['module:village:organizeAdd']"
        >发布</el-button>
      </el-col>
    <!--  <el-col :span="1.5">
        <el-button
...
<script>
  ...
  import {
    ...
    putVillages, organizeList,
    getVillage, delVillage, organizeAdd,
    updateVillage, organizeExport
...
//============================================================
      ...
      getList() {
        this.loading = true;
        organizeList(this.queryParams).then(response => {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.id != undefined) {
              ...
              this.form.img = JSON.stringify(images);
              this.form.video = JSON.stringify(videos);
              organizeAdd(this.form).then(response => {
                if (response.code === 200) {
      ...
      handleDelete(row) {
        ...
        this.$confirm('是否确认删除组织振兴信息为"' + row.title + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
      ...
      handleExport() {
        ...
        this.$confirm('是否确认导出所有组织振兴信息数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function () {
          return organizeExport(queryParams);
        }).then(response => {
